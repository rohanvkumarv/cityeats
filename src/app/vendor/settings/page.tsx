...
export default function VendorSettings() {
  ...
  const handleImageUpload = async (e) => {
    ...
    if (!restaurant) {
      showMessage('Please save restaurant details first', 'error');
      return;
    }
    ...
    if (oversizedFiles.length > 0) {
      showMessage('Some files are larger than 5MB. Please choose smaller files.', 'error');
      return;
    }
    
    if (invalidTypes.length > 0) {
      showMessage('Please select only image files (JPG, PNG, GIF)', 'error');
      return;
    }
    ...
    try {
      ...
      xhr.open('POST', '/api/vendor/upload-images');
      xhr.send(formData);
      
      const data = await uploadPromise;
      
      if (data.success) {
        // Refresh restaurant data to get updated images
  ...
  const setMainImage = async (imageId) => {
    try {
      const res = await fetch(`/api/vendor/images/${imageId}/set-main`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        }
      ...
      if (data.success) {
        ...
        showMessage(data.error || 'Failed to update main image', 'error');
      }
    } catch (error) {
      console.error('Error setting main image:', error);
      showMessage('Failed to update main image', 'error');
    }
  };
  ...
  const deleteImage = async (imageId) => {
    ...
    try {
      const res = await fetch(`/api/vendor/images/${imageId}/delete`, {
        method: 'DELETE'
      });
        ...
        {activeTab === 'images' && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-6 border border-stone-100">
            ...
            <div className="mb-6">
              ...
              <div className="border-2 border-dashed border-stone-200 rounded-lg p-6 text-center hover:border-amber-300 transition-colors">
                <input
                  type="file"
                  multiple
                  accept="image/*"
                ...
                >
                  <RiImageAddLine className={`text-5xl mb-3 ${imageUploading ? 'text-amber-400' : 'text-amber-300'}`} />
            ...
            {restaurant?.images && restaurant.images.length > 0 ? (
              <div>
                <h3 className="font-medium mb-3 text-amber-900">Current Images ({restaurant.images.length})</h3>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {restaurant.images.map((image, index) => (
                    <div key={image.id} className="relative rounded-lg overflow-hidden h-40 bg-stone-100 border border-stone-200 group">
                      <img
                        src={image.imageUrl}
                      ...
                      />
                      {image.isMain && (
                        <div className="absolute top-2 left-2 bg-amber-600 text-white text-xs px-2 py-1 rounded-full flex items-center">
                        ...
                        <div className="flex space-x-2">
                          {!image.isMain && (
                            <button
                              ...
                              className="bg-amber-600 text-white p-2 rounded-full hover:bg-amber-700 transition-all duration-300"
              ...
              <div className="text-center text-stone-500 py-8">
                <RiImageAddLine className="mx-auto h-12 w-12 text-stone-300 mb-3" />
                <p className="font-medium">No images uploaded yet</p>
                <p className="text-sm">Upload images to showcase your restaurant</p>
              </div>
            )}
          </div>
        )}
        
        {/* Submit Button - Only show on basic and tables tabs */}
        {(activeTab === 'basic' || activeTab === 'tables') && (
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={saving}
